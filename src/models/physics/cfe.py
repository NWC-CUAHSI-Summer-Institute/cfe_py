...
class CFE():
    ...
    def calculate_evaporation_from_rainfall(self, cfe_state):
        ...
        cfe_state.vol_et_from_rain = torch.add(cfe_state.vol_et_from_rain, cfe_state.actual_et_from_rain_m_per_timestep)
        cfe_state.vol_et_to_atm = torch.add(cfe_state.vol_et_to_atm, cfe_state.actual_et_from_rain_m_per_timestep)
        cfe_state.volout = torch.add(cfe_state.volout, cfe_state.actual_et_from_rain_m_per_timestep)
        
        cfe_state.actual_et_m_per_timestep = torch.add(cfe_state.actual_et_m_per_timestep, cfe_state.actual_et_from_rain_m_per_timestep)
    ...
    def calculate_evaporation_from_soil(self, cfe_state):
        ...
        if cfe_state.soil_params['scheme'].lower() == 'classic':
            ...
            cfe_state.vol_et_to_atm = torch.add(cfe_state.vol_et_to_atm, cfe_state.actual_et_from_soil_m_per_timestep)
            cfe_state.volout = torch.add(cfe_state.volout, cfe_state.actual_et_from_soil_m_per_timestep)

            cfe_state.actual_et_m_per_timestep = torch.add(cfe_state.actual_et_m_per_timestep, cfe_state.actual_et_from_soil_m_per_timestep)
    ...
    def calculate_saturation_excess_overland_flow_from_soil(self, cfe_state):
        ...
        if cfe_state.soil_reservoir_storage_deficit_m < cfe_state.infiltration_depth_m:
            ...
            cfe_state.surface_runoff_depth_m = torch.add(cfe_state.surface_runoff_depth_m, diff)
            cfe_state.infiltration_depth_m = torch.sub(cfe_state.infiltration_depth_m, diff)
    ...
    def track_infiltration_and_runoff(self, cfe_state):
        ...
        """
        cfe_state.vol_partition_runoff = torch.add(cfe_state.vol_partition_runoff, cfe_state.surface_runoff_depth_m)
        cfe_state.vol_partition_infilt = torch.add(cfe_state.vol_partition_infilt, cfe_state.infiltration_depth_m)
        cfe_state.vol_to_soil = torch.add(cfe_state.vol_to_soil, cfe_state.infiltration_depth_m)
    ...
    def run_soil_moisture_scheme(self, cfe_state):
        ...
        if cfe_state.soil_params['scheme'].lower() == 'classic':
            ...
            cfe_state.soil_reservoir['storage_m'] = torch.add(cfe_state.soil_reservoir['storage_m'], cfe_state.infiltration_depth_m)
    ...
    def update_outflux_from_soil(self, cfe_state):
        ...
        if cfe_state.soil_params['scheme'].lower() == 'classic':
            cfe_state.soil_reservoir['storage_m'] = torch.sub(cfe_state.soil_reservoir['storage_m'], cfe_state.flux_perc_m)
            cfe_state.soil_reservoir['storage_m'] = torch.sub(cfe_state.soil_reservoir['storage_m'], cfe_state.flux_lat_m)
...
>>>>>>> d682d7c4dbc9df9dc9fb33ea23a5d2a539ecf7d1
    ...
    def calculate_saturation_excess_overland_flow_from_gw(self, cfe_state):
        ...
        if cfe_state.flux_perc_m > cfe_state.gw_reservoir_storage_deficit_m:
            ...
            cfe_state.vol_partition_runoff = torch.add(cfe_state.vol_partition_runoff, diff)
            cfe_state.vol_partition_infilt = torch.sub(cfe_state.vol_partition_infilt, diff)
            
        cfe_state.gw_reservoir['storage_m'] = torch.add(cfe_state.gw_reservoir['storage_m'], cfe_state.flux_perc_m)
...
>>>>>>> d682d7c4dbc9df9dc9fb33ea23a5d2a539ecf7d1
    ...
    def set_flux_from_deep_gw_to_chan_m(self, cfe_state):
        ...
        cfe_state.vol_from_gw = torch.add(cfe_state.vol_from_gw, cfe_state.flux_from_deep_gw_to_chan_m)
        
    # __________________________________________________________________________________________________________
    def remove_flux_from_deep_gw_to_chan_m(self, cfe_state):
        """ Just an accounting operation
        """
        cfe_state.gw_reservoir['storage_m'] = torch.sub(cfe_state.gw_reservoir['storage_m'], cfe_state.flux_from_deep_gw_to_chan_m)
    # __________________________________________________________________________________________________________
    def track_volume_from_giuh(self, cfe_state):
        cfe_state.vol_out_giuh = torch.add(cfe_state.vol_out_giuh, cfe_state.flux_giuh_runoff_m)
        cfe_state.volout = torch.add(cfe_state.volout, cfe_state.flux_giuh_runoff_m)
    # __________________________________________________________________________________________________________
    def track_volume_from_deep_gw_to_chan(self, cfe_state):
        cfe_state.volout = torch.add(cfe_state.volout, cfe_state.flux_from_deep_gw_to_chan_m )
    # __________________________________________________________________________________________________________
    def track_volume_from_nash_cascade(self, cfe_state):
        cfe_state.vol_in_nash = torch.add(cfe_state.vol_in_nash, cfe_state.flux_lat_m)
        cfe_state.vol_out_nash = torch.add(cfe_state.vol_out_nash, cfe_state.flux_nash_lateral_runoff_m)
    # __________________________________________________________________________________________________________
    ...
    def nash_cascade(self,cfe_state):
        ...
        for i in range(cfe_state.num_lateral_flow_nash_reservoirs):
            ...
            cfe_state.nash_storage[i] = torch.sub(cfe_state.nash_storage[i], Q[i])
            
            if i == 0:
                
                cfe_state.nash_storage[i] = torch.add(cfe_state.nash_storage[i], cfe_state.flux_lat_m)
                
            else:
                
                cfe_state.nash_storage[i] = torch.add(cfe_state.nash_storage[i], Q[i-1])
    ...
    def convolution_integral(self,cfe_state):
        ...
        for i in range(cfe_state.num_giuh_ordinates): 
            ...
            cfe_state.runoff_queue_m_per_timestep[i] = torch.add(cfe_state.runoff_queue_m_per_timestep[i], cfe_state.giuh_ordinates[i] * cfe_state.surface_runoff_depth_m)
    ...
    def et_from_rainfall(self,cfe_state):
        ...
        if cfe_state.timestep_rainfall_input_m > cfe_state.potential_et_m_per_timestep:
            ...
            cfe_state.timestep_rainfall_input_m = torch.sub(cfe_state.timestep_rainfall_input_m, cfe_state.actual_et_from_rain_m_per_timestep)
    ...
    def et_from_soil(self,cfe_state):
        ...
        if cfe_state.reduced_potential_et_m_per_timestep > 0:
            ...
            cfe_state.soil_reservoir['storage_m'] = torch.sub(cfe_state.soil_reservoir['storage_m'], cfe_state.actual_et_from_soil_m_per_timestep)
            cfe_state.reduced_potential_et_m_per_timestep = torch.sub(cfe_state.reduced_potential_et_m_per_timestep, cfe_state.actual_et_from_soil_m_per_timestep)
