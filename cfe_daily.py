...
class CFE():
    ...
    # __________________________________________________________________________________________________________
    def track_volume_from_nash_cascade_surface(self, cfe_state):
        cfe_state.vol_in_nash_surface += cfe_state.surface_runoff_depth_m
        cfe_state.vol_out_nash_surface += cfe_state.flux_nash_surface_runoff_m
    # __________________________________________________________________________________________________________
    def track_volume_from_deep_gw_to_chan(self, cfe_state):
        cfe_state.volout += cfe_state.flux_from_deep_gw_to_chan_m 
    # __________________________________________________________________________________________________________
    def track_volume_from_nash_cascade_lateral(self, cfe_state):
        cfe_state.vol_in_nash_lateral += cfe_state.flux_lat_m
        cfe_state.vol_out_nash_lateral += cfe_state.flux_nash_lateral_runoff_m
    # __________________________________________________________________________________________________________
    def add_up_total_flux_discharge(self, cfe_state):
        ...
        cfe_state.flux_Qout_m = cfe_state.flux_nash_surface_runoff_m + cfe_state.flux_nash_lateral_runoff_m + cfe_state.flux_from_deep_gw_to_chan_m
    ...
    def run_cfe(self, cfe_state):
        ...
        self.track_volume_from_nash_cascade_surface(cfe_state)
        self.track_volume_from_deep_gw_to_chan(cfe_state)
        
        # Lateral flow routing
        self.nash_cascade_lateral(cfe_state)
        self.track_volume_from_nash_cascade_lateral(cfe_state)
    ...
    # __________________________________________________________________________________________________________
    def nash_cascade_lateral(self,cfe_state):
        """
        ...
        for i in range(cfe_state.num_lateral_flow_nash_reservoirs):
            ...
            Q[i] = cfe_state.K_nash_lateral * cfe_state.nash_storage_lateral[i]
            
            cfe_state.nash_storage_lateral[i] -= Q[i]
            
            if i == 0:
                
                cfe_state.nash_storage_lateral[i] += cfe_state.flux_lat_m
                
            else:
                
                cfe_state.nash_storage_lateral[i] += Q[i-1]
    ...
    def nash_cascade_surface(self,cfe_state):
        """
            ...
            arrival of the lateral flow into the channel
        """
        Q = np.zeros(cfe_state.num_surface_flow_nash_reservoirs)
        
        for i in range(cfe_state.num_surface_flow_nash_reservoirs):
            
            Q[i] = cfe_state.K_nash_surface * cfe_state.nash_storage_surface[i]
            
            cfe_state.nash_storage_surface[i] -= Q[i]
            
            if i == 0:
                
                cfe_state.nash_storage_surface[i] += cfe_state.surface_runoff_depth_m
                
            else:
                
                cfe_state.nash_storage_surface[i] += Q[i-1]
        
        cfe_state.flux_nash_surface_runoff_m = Q[cfe_state.num_surface_flow_nash_reservoirs - 1]
